...
pub fn extract_node_id_from_cert(cert: &CertificateDer<'_>) -> Result<NodeId, Error> {
    ...
    if cert_bytes.len() < 32 {
        return Err(Error::General("Certificate too short for Ed25519 key".into()));
    }
...
pub fn verify_self_signed(cert: &CertificateDer<'_>, expected_node_id: &NodeId) -> Result<(), Error> {
