...
use crate::{Result, GossipError, MessageType};
...
impl ReplyTo {
    /// Create a new ReplyTo handle
    pub(crate) fn new(correlation_id: u16, connection: Arc<crate::connection_pool::ConnectionHandle>) -> Self {
        Self {
            correlation_id,
            connection,
        }
    ...
    pub async fn reply(self, response: &[u8]) -> Result<()> {
        ...
        tracing::info!("ReplyTo::reply: sending response with correlation_id={} on connection to {}", 
                      self.correlation_id, self.connection.addr);
