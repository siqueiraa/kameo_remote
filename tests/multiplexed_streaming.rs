...
async fn test_basic_multiplexed_streaming(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📤 Testing multiplexed stream write: {:?}", std::str::from_utf8(test_data));
    
    // Test non-blocking write
    let result = stream_handle.write_nonblocking(test_data);
    assert!(result.is_ok(), "Lock-free stream write failed: {:?}", result.err());
    ...
    println!("✅ Basic multiplexed streaming: {} bytes written", stream_handle.bytes_written());
...
async fn test_concurrent_streaming_tell(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing concurrent streaming + tell with {} iterations", ITERATIONS);
    ...
    println!("   📤 Stream operations: {}/{} successful", stream_success, ITERATIONS);
...
async fn test_multiple_streaming_channels(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing multiple streaming channels with {} iterations each", ITERATIONS);
...
async fn test_channel_performance_comparison(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Comparing channel performance with {}KB buffers", BUFFER_SIZE / 1024);
    ...
    let tell_throughput = (tell_success * BUFFER_SIZE) as f64 * 8.0 / tell_duration.as_secs_f64() / 1_000_000.0;
    let stream_throughput = stream_bytes as f64 * 8.0 / stream_duration.as_secs_f64() / 1_000_000.0;
    let priority_throughput = priority_bytes as f64 * 8.0 / priority_duration.as_secs_f64() / 1_000_000.0;
    
    println!("\n📊 Channel Performance Comparison:");
    println!("   📤 Regular tell():     {:?} ({:.2} Mbps) - {}/{} success", 
             tell_duration, tell_throughput, tell_success, ITERATIONS);
    println!("   🔄 Stream channel:     {:?} ({:.2} Mbps) - {} bytes", 
             stream_duration, stream_throughput, stream_bytes);
    println!("   ⚡ Priority channel:   {:?} ({:.2} Mbps) - {} bytes", 
             priority_duration, priority_throughput, priority_bytes);
    
    let best_throughput = tell_throughput.max(stream_throughput).max(priority_throughput);
    println!("   🏆 Best throughput: {:.2} Mbps ({:.2} MB/s)", best_throughput, best_throughput / 8.0);
}
...
async fn test_high_throughput_multiplexed(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing high-throughput multiplexed streaming for {:?}", STREAM_DURATION);
    ...
    println!("   📤 Total bytes: {} ({:.2} GB)", bulk_handle.bytes_written(), total_bytes / 1_000_000_000.0);
    ...
    if throughput_mbps > 10000.0 {
        ...
        println!("   ℹ️  Multiplexed performance: {:.2} Mbps", throughput_mbps);
    }
}
