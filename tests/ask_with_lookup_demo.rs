...
async fn test_ask_with_lookup_and_performance() {
    ...
    peer2.connect(&"127.0.0.1:30002".parse().unwrap()).await.unwrap();
    peer3.connect(&"127.0.0.1:30003".parse().unwrap()).await.unwrap();
    ...
    peer1_from_2.connect(&"127.0.0.1:30001".parse().unwrap()).await.unwrap();
    peer3_from_2.connect(&"127.0.0.1:30003".parse().unwrap()).await.unwrap();
    ...
    peer1_from_3.connect(&"127.0.0.1:30001".parse().unwrap()).await.unwrap();
    peer2_from_3.connect(&"127.0.0.1:30002".parse().unwrap()).await.unwrap();
    ...
    let db_conn = node1.get_connection(match db_location.peer_id.as_str() {
    ...
    let compute_conn = node1.get_connection(match compute_location.peer_id.as_str() {
        "node1" => "127.0.0.1:30001".parse().unwrap(),
        "node2" => "127.0.0.1:30002".parse().unwrap(),
        "node3" => "127.0.0.1:30003".parse().unwrap(),
        _ => panic!("Unknown peer"),
    ...
    let cache_conn = node1.get_connection(match cache_location.peer_id.as_str() {
        "node1" => "127.0.0.1:30001".parse().unwrap(),
        "node2" => "127.0.0.1:30002".parse().unwrap(),
        "node3" => "127.0.0.1:30003".parse().unwrap(),
        _ => panic!("Unknown peer"),
    ...
    println!("     - database_service at {} (peer: {})", db_location.address, db_location.peer_id);
    println!("     - compute_service at {} (peer: {})", compute_location.address, compute_location.peer_id);
    println!("     - cache_service at {} (peer: {})", cache_location.address, cache_location.peer_id);
    println!("   📈 Total lookup time: {:?} ({:.3} μs)", lookup_time, lookup_time.as_nanos() as f64 / 1000.0);
    ...
    for (i, (service, query)) in queries.iter().enumerate() {
        ...
        println!("     Query {} response: \"{}\" in {:?} ({:.3} μs)", 
                i+1, response_str, ask_time, ask_time.as_nanos() as f64 / 1000.0);
    }
    ...
    println!("     - Average per query: {:?} ({:.3} μs)", individual_avg, individual_avg.as_nanos() as f64 / 1000.0);
    ...
    println!("     - Total time: {:?} ({:.3} μs)", parallel_total, parallel_total.as_nanos() as f64 / 1000.0);
    println!("     - Average per query: {:?} ({:.3} μs)", parallel_avg, parallel_avg.as_nanos() as f64 / 1000.0);
    println!("     - Speedup vs sequential: {:.2}x", individual_total.as_nanos() as f64 / parallel_total.as_nanos() as f64);
    ...
    println!("\n🔸 Test 3A: tell() performance ({} iterations)", iterations);
    ...
    println!("     - Total: {:?} ({:.3} μs)", tell_total, tell_total.as_nanos() as f64 / 1000.0);
    println!("     - Average: {:?} ({:.3} μs)", tell_avg, tell_avg.as_nanos() as f64 / 1000.0);
    println!("     - Throughput: {:.0} ops/sec", iterations as f64 / tell_total.as_secs_f64());
    
    println!("\n🔸 Test 3B: ask() performance ({} iterations)", iterations);
    let ask_start = Instant::now();
    for _ in 0..iterations {
    ...
    println!("   📈 ask() Results:");
    println!("     - Total: {:?} ({:.3} μs)", ask_total, ask_total.as_nanos() as f64 / 1000.0);
    println!("     - Average: {:?} ({:.3} μs)", ask_avg, ask_avg.as_nanos() as f64 / 1000.0);
    println!("     - Throughput: {:.0} ops/sec", iterations as f64 / ask_total.as_secs_f64());
    
    let ask_overhead = ask_total.as_nanos() as f64 / tell_total.as_nanos() as f64;
    println!("\n   🎯 ask() overhead: {:.2}x slower than tell()", ask_overhead);
    ...
    println!("   - Mock response: \"{}\"", String::from_utf8_lossy(&mock_response));
    
    // Test with timeout
    println!("\n🔸 Test 4B: ask_with_timeout_and_reply()");
    let timeout = Duration::from_millis(100);
    let reply_with_timeout = db_conn.ask_with_timeout_and_reply(request, timeout).await.unwrap();
...
async fn test_ask_high_throughput() {
    ...
    for batch in 0..(request_count / concurrent_requests) {
        ...
        if batch % 10 == 0 {
            println!("   - Completed {} requests", (batch + 1) * concurrent_requests);
        }
    }
    ...
    println!("   - Average latency: {:?} ({:.3} μs)", avg_latency, avg_latency.as_nanos() as f64 / 1000.0);
