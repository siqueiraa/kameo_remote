...
async fn test_keypair_generation_and_peer_id() {
    ...
    assert_ne!(peer_id1, peer_id2, "Different keypairs should produce different PeerIds");
    
    // Verify that the same keypair consistently produces the same PeerId
    let peer_id1_again = keypair1.peer_id();
    assert_eq!(peer_id1, peer_id1_again, "Same keypair should produce same PeerId");
    
    // Verify key bytes are correct length
    assert_eq!(keypair1.public_key_bytes().len(), 32, "Public key should be 32 bytes");
    assert_eq!(keypair1.private_key_bytes().len(), 32, "Private key should be 32 bytes");
...
async fn test_deterministic_keypair_for_testing() {
    ...
    assert_eq!(keypair1.peer_id(), keypair2.peer_id(), "Same seed should produce same PeerId");
    assert_eq!(keypair1.public_key_bytes(), keypair2.public_key_bytes(), "Same seed should produce same public key");
    assert_eq!(keypair1.private_key_bytes(), keypair2.private_key_bytes(), "Same seed should produce same private key");
    
    // Different seeds should produce different keypairs
    assert_ne!(keypair1.peer_id(), keypair3.peer_id(), "Different seeds should produce different PeerIds");
...
async fn test_peer_id_conversions() {
    ...
    assert_eq!(hex_string.len(), 64, "Hex string should be 64 characters (32 bytes * 2)");
    ...
    assert_eq!(display_string, hex_string, "Display should match hex string");
    assert_eq!(peer_id.as_str(), hex_string, "as_str should match hex string");
...
async fn test_key_mismatch_connection_rejection() {
    ...
    let server_registry = GossipRegistryHandle::new(
        server_addr,
        vec![],
        Some(server_config)
    ).await.expect("Should create server registry");
    ...
    let client_registry = GossipRegistryHandle::new(
        client_addr,
        vec![],
        Some(client_config)
    ).await.expect("Should create client registry");
    
    println!("   💻 Client started with PeerId: {}", client_peer_id);
    assert_ne!(server_peer_id, client_peer_id, "Client and server should have different PeerIds");
    ...
    match timeout(Duration::from_secs(5), wrong_peer.connect(&server_addr)).await {
        ...
        Ok(Err(e)) => println!("      ✅ Connection with wrong PeerId correctly failed: {}", e),
    ...
    assert_ne!(server_peer_id, wrong_peer_id, "Server and wrong PeerIds should be different");
    assert_ne!(client_peer_id, server_peer_id, "Client and server PeerIds should be different");
    assert_ne!(client_peer_id, wrong_peer_id, "Client and wrong PeerIds should be different");
...
async fn test_invalid_key_edge_cases() {
    ...
    for (i, invalid_hex) in invalid_hex_cases.iter().enumerate() {
        println!("   Test case {}: '{}'", i + 1, if invalid_hex.len() > 20 { &format!("{}...", &invalid_hex[..20]) } else { invalid_hex });
    ...
    for (i, invalid_private) in invalid_private_key_cases.iter().enumerate() {
        println!("   Private key test case {}: {} bytes", i + 1, invalid_private.len());
        match KeyPair::from_private_key_bytes(invalid_private) {
            Ok(_) => println!("      ❌ ERROR: Should have failed to create KeyPair from invalid private key"),
...
async fn test_legacy_string_compatibility() {
    ...
    assert_eq!(peer_id_from_string, peer_id_from_string_again, 
    ...
    assert_ne!(peer_id_from_string, different_peer_id,
    ...
    assert_eq!(peer_id_from_string, peer_id_from_hex,
