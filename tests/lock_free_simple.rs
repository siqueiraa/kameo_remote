...
async fn test_lock_free_basic() {
    ...
    println!("   📊 After processing: {} pending, {} available", pending_after, available_after);
...
async fn test_lock_free_concurrent() {
    ...
    println!("📊 Testing concurrent lock-free + tell with {} iterations", ITERATIONS);
    ...
    println!("   📤 Lock-free operations: {}/{} successful", stream_success, ITERATIONS);
...
async fn test_lock_free_performance() {
    ...
    println!("📊 Testing high-throughput lock-free streaming for {:?}", STREAM_DURATION);
    ...
    println!("   📤 Total bytes: {} ({:.2} MB)", lock_free_handle.bytes_written(), total_bytes / 1_000_000.0);
