...
impl StreamingMetrics {
    ...
    pub fn finalize(&mut self) {
        if self.total_duration.as_nanos() > 0 {
            self.throughput_mbps = (self.total_bytes_sent as f64 * 8.0) / (self.total_duration.as_nanos() as f64 / 1_000_000_000.0) / 1_000_000.0;
            self.messages_per_second = self.total_messages_sent as f64 / self.total_duration.as_secs_f64();
        }
    ...
    pub fn print_results(&self, test_name: &str) {
        ...
        println!("   📤 Total Bytes Sent: {} bytes ({:.2} MB)", self.total_bytes_sent, self.total_bytes_sent as f64 / 1_000_000.0);
...
async fn test_small_message_latency(conn: &crate::connection_pool::ConnectionHandle) -> StreamingMetrics {
...
async fn test_large_message_throughput(conn: &crate::connection_pool::ConnectionHandle) -> StreamingMetrics {
    let mut metrics = StreamingMetrics::new();
    let iterations = 100;
    let mut total_latency = Duration::from_nanos(0);
...
async fn test_raw_stream_performance(conn: &crate::connection_pool::ConnectionHandle) -> StreamingMetrics {
    let mut metrics = StreamingMetrics::new();
    let iterations = 1000;
...
async fn test_performance_comparison(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("   📤 Regular tell():     {:?} ({:.3} μs avg)", tell_duration, tell_duration.as_nanos() as f64 / iterations as f64 / 1000.0);
    println!("   🔄 Streaming send():   {:?} ({:.3} μs avg)", stream_duration, stream_duration.as_nanos() as f64 / iterations as f64 / 1000.0);
    println!("   ⚡ Raw stream handle: {:?} ({:.3} μs avg)", raw_duration, raw_duration.as_nanos() as f64 / iterations as f64 / 1000.0);
    ...
    println!("   📈 Streaming overhead: {:.2}x vs tell()", stream_overhead);
    println!("   🚀 Raw stream speedup: {:.2}x vs tell()", raw_improvement);
    
    // Throughput calculations
    let tell_throughput = (iterations * test_data.len()) as f64 / tell_duration.as_secs_f64() / 1_000_000.0;
    let stream_throughput = (iterations * stream_data.size()) as f64 / stream_duration.as_secs_f64() / 1_000_000.0;
    let raw_throughput = (iterations * test_data.len()) as f64 / raw_duration.as_secs_f64() / 1_000_000.0;
...
async fn test_streaming_basic_functionality() {
    ...
    assert!(result.is_ok(), "Batch stream send failed: {:?}", result.err());
    ...
    assert!(result.is_ok(), "Raw stream write failed: {:?}", result.err());
    
    println!("✅ Raw stream write successful");
    
    // Test vectored write
    let bufs = vec![b"Hello, ".as_slice(), b"vectored ".as_slice(), b"write!".as_slice()];
