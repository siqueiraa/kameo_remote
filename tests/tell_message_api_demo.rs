...
async fn test_tell_message_api_comprehensive() {
    ...
    for (i, msg) in chat_messages.iter().enumerate() {
        ...
        println!("     Message {}: {:?} μs", i+1, elapsed.as_nanos() as f64 / 1000.0);
    }
    let single_total = single_start.elapsed();
    
    println!("   📈 Single Message Results:");
    println!("     - Total time: {:?} ({:.3} μs)", single_total, single_total.as_nanos() as f64 / 1000.0);
    println!("     - Average per message: {:?} ({:.3} μs)", single_total / 5, single_total.as_nanos() as f64 / 5000.0);
    ...
    println!("     - Total payload: {} bytes", batch_data.iter().map(|msg| msg.len()).sum::<usize>());
    ...
    println!("     - Total time: {:?} ({:.3} μs)", batch_total, batch_total.as_nanos() as f64 / 1000.0);
    println!("     - Average per message: {:?} ({:.3} μs)", batch_total / 5, batch_total.as_nanos() as f64 / 5000.0);
    println!("     - Method: 1 batched conn.tell() call");
    
    let improvement = single_total.as_nanos() as f64 / batch_total.as_nanos() as f64;
    println!("   🎯 Performance Improvement: {:.2}x faster with batching", improvement);
    ...
    for (i, (name, data)) in file_data.iter().enumerate() {
        ...
        let elapsed = start.elapsed();
        println!("     File {}: {} transferred in {:?} ({:.3} μs)", i+1, name, elapsed, elapsed.as_nanos() as f64 / 1000.0);
    }
    let file_single_total = file_single_start.elapsed();
    
    let total_bytes: usize = file_data.iter().map(|(_, data)| data.len()).sum();
    let single_throughput = total_bytes as f64 / file_single_total.as_secs_f64() / (1024.0 * 1024.0);
    
    println!("   📈 Individual File Transfer Results:");
    println!("     - Total time: {:?} ({:.3} μs)", file_single_total, file_single_total.as_nanos() as f64 / 1000.0);
    println!("     - Total data: {} bytes ({:.2} KB)", total_bytes, total_bytes as f64 / 1024.0);
    ...
    println!("     - Total payload: {} bytes ({:.2} KB)", total_bytes, total_bytes as f64 / 1024.0);
    
    let file_batch_start = Instant::now();
    conn.tell(kameo_remote::connection_pool::TellMessage::batch(file_batch_data)).await.unwrap();
    ...
    println!("     - Total time: {:?} ({:.3} μs)", file_batch_total, file_batch_total.as_nanos() as f64 / 1000.0);
    println!("     - Total data: {} bytes ({:.2} KB)", total_bytes, total_bytes as f64 / 1024.0);
    ...
    println!("   🎯 Performance Improvement: {:.2}x faster, {:.2}x better throughput", file_improvement, throughput_improvement);
    ...
    println!("     - Actor registered and propagated in: {:?} ({:.3} μs)", gossip_time, gossip_time.as_nanos() as f64 / 1000.0);
    ...
    println!("     - {} messages sent in: {:?} ({:.3} μs)", post_gossip_messages.len(), mixed_time, mixed_time.as_nanos() as f64 / 1000.0);
    println!("     - Average per message: {:?} ({:.3} μs)", mixed_time / 3, mixed_time.as_nanos() as f64 / 3000.0);
    ...
    for (i, request) in api_requests.iter().enumerate() {
        ...
        println!("     Request {}: {:?} -> {:?} in {:?} ({:.3} μs)", 
                i+1, request, response_str, req_time, req_time.as_nanos() as f64 / 1000.0);
    }
    ...
    println!("     - Total time: {:?} ({:.3} μs)", ask_total, ask_total.as_nanos() as f64 / 1000.0);
    println!("     - Average per request: {:?} ({:.3} μs)", ask_avg, ask_avg.as_nanos() as f64 / 1000.0);
    ...
    println!("     - Request: {:?} ({} bytes)", std::str::from_utf8(volume_request).unwrap(), volume_request.len());
    ...
    println!("     - Total time: {:?} ({:.3} μs)", volume_total, volume_total.as_nanos() as f64 / 1000.0);
    println!("     - Average latency: {:?} ({:.3} μs)", volume_avg, volume_avg.as_nanos() as f64 / 1000.0);
    println!("     - Requests/sec: {:.0}", volume_rps);
    println!("     - P50 latency: {:?} ({:.3} μs)", p50, p50.as_nanos() as f64 / 1000.0);
    println!("     - P95 latency: {:?} ({:.3} μs)", p95, p95.as_nanos() as f64 / 1000.0);
    println!("     - P99 latency: {:?} ({:.3} μs)", p99, p99.as_nanos() as f64 / 1000.0);
    ...
    println!("     - Message: {:?} ({} bytes)", std::str::from_utf8(comparison_request).unwrap(), comparison_request.len());
    ...
    println!("       - Total time: {:?} ({:.3} μs)", tell_total, tell_total.as_nanos() as f64 / 1000.0);
    println!("       - Average latency: {:?} ({:.3} μs)", tell_avg, tell_avg.as_nanos() as f64 / 1000.0);
    println!("       - Operations/sec: {:.0}", tell_rps);
    println!("     ask() (request-response):");
    println!("       - Total time: {:?} ({:.3} μs)", ask_comp_total, ask_comp_total.as_nanos() as f64 / 1000.0);
    println!("       - Average latency: {:?} ({:.3} μs)", ask_comp_avg, ask_comp_avg.as_nanos() as f64 / 1000.0);
    println!("       - Operations/sec: {:.0}", ask_comp_rps);
    
    let ask_overhead = ask_comp_total.as_nanos() as f64 / tell_total.as_nanos() as f64;
    println!("     🎯 ask() overhead: {:.2}x slower than tell() (expected due to response handling)", ask_overhead);
    ...
    println!("   2. **ask() - Request-response**: Simple API for synchronous communication patterns");
    println!("   3. **Performance**: Batching provides {:.1}x-{:.1}x speedup for multiple messages", improvement.min(file_improvement), improvement.max(file_improvement));
...
async fn test_tell_message_high_volume_performance() {
    ...
    let individual_throughput = (message_count * message_size) as f64 / individual_time.as_secs_f64() / (1024.0 * 1024.0);
    ...
    println!("     - Avg latency: {:.3} μs", individual_time.as_nanos() as f64 / message_count as f64 / 1000.0);
    ...
    for i in 0..batch_count {
        ...
        conn.tell(kameo_remote::connection_pool::TellMessage::batch(batch_data)).await.unwrap();
        if i % 4 == 0 {
            println!("   - Sent batch {}/{} ({} messages)", i, batch_count, i * batch_size);
        }
    }
    let batch_time = batch_start.elapsed();
    
    let batch_throughput = (message_count * message_size) as f64 / batch_time.as_secs_f64() / (1024.0 * 1024.0);
    ...
    println!("     - Throughput: {:.2} MB/s", batch_throughput);
    println!("     - Avg latency: {:.3} μs", batch_time.as_nanos() as f64 / message_count as f64 / 1000.0);
    ...
    println!("   - Batch is {:.2}x faster than individual messages", speedup);
    println!("   - Throughput improvement: {:.2}x", throughput_improvement);
    println!("   - Time saved: {:?} ({:.1}% reduction)", individual_time - batch_time, (1.0 - batch_time.as_secs_f64() / individual_time.as_secs_f64()) * 100.0);
    ...
    assert!(speedup > 5.0, "Batching should provide at least 5x speedup for high-volume data");
    assert!(throughput_improvement > 3.0, "Batching should provide at least 3x throughput improvement");
