...
async fn test_batch_ask_simple() -> Result<()> {
    ...
    // Test 2: Small batch
    {
        let requests: Vec<Vec<u8>> = (0..10u32)
    ...
    // Test 3: Large batch
    {
        ...
        info!("Batch ask ({}): completed in {:?} ({:.2} req/sec)", 
    ...
    // Test 4: Concurrent batches
    {
        ...
        for task_id in 0..NUM_CONCURRENT {
            ...
            let task = tokio::spawn(async move {
                ...
                for receiver in receivers {
                    match receiver.await {
                        Ok(response) => responses.push(response),
                        Err(_) => return Err(kameo_remote::GossipError::Network(
                            std::io::Error::new(std::io::ErrorKind::Other, "Failed to receive response")
                        )),
                    }
                }
        ...
        info!("Concurrent batch asks ({} tasks x {} requests): completed in {:?} ({:.2} req/sec)",
              NUM_CONCURRENT, BATCH_SIZE, elapsed, 
...
async fn test_batch_ask_with_timeout() -> Result<()> {
    ...
    let requests: Vec<Vec<u8>> = (0..5u32)
        .map(|i| i.to_be_bytes().to_vec())
        .collect();
    let request_refs: Vec<&[u8]> = requests.iter()
        .map(|r| r.as_slice())
    ...
    let results = conn.ask_batch_with_timeout(&request_refs, Duration::from_secs(1)).await?;
