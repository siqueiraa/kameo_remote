...
async fn test_baseline_tcp_performance(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing baseline TCP with {}KB buffers, {} iterations", 
    ...
    println!("   📤 Successful writes: {}/{}", successful_writes, ITERATIONS);
    ...
    println!("   ⚡ Avg per write: {:.2} ms", elapsed.as_millis() as f64 / successful_writes as f64);
...
async fn test_optimized_tcp_batching(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing TCP batching with {} buffers of {}KB each per batch", 
             BUFFERS_PER_BATCH, BUFFER_SIZE / 1024);
    ...
    println!("   📤 Successful batches: {}/{}", successful_batches, ITERATIONS);
    println!("   📤 Total bytes: {:.2} MB", total_bytes / 1_000_000.0);
    println!("   ⏱️  Duration: {:?}", elapsed);
    println!("   🚀 Throughput: {:.2} Mbps ({:.2} MB/s)", throughput_mbps, throughput_mbps / 8.0);
    println!("   ⚡ Avg per batch: {:.2} ms", elapsed.as_millis() as f64 / successful_batches as f64);
}
...
async fn test_large_buffer_streaming(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing large buffer streaming with {}KB buffers", BUFFER_SIZE / 1024);
...
async fn test_sustained_tcp_throughput(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing sustained TCP throughput for {:?} with {}KB buffers", 
             TEST_DURATION, BUFFER_SIZE / 1024);
    ...
    println!("   📤 Total bytes: {:.2} MB", total_bytes / 1_000_000.0);
    println!("   ⏱️  Duration: {:?}", elapsed);
    println!("   🚀 Sustained Throughput: {:.2} Mbps ({:.2} MB/s)", throughput_mbps, throughput_mbps / 8.0);
    println!("   📊 Writes/sec: {:.2}", writes_per_second);
    println!("   ⚡ Avg per write: {:.2} ms", elapsed.as_millis() as f64 / successful_writes as f64);
    println!("   📊 Success rate: {:.1}%", (successful_writes as f64 / (successful_writes + total_errors) as f64) * 100.0);
...
async fn test_realistic_performance_comparison() {
    ...
    println!("📊 Comparing different TCP streaming approaches with {}KB buffers", BUFFER_SIZE / 1024);
    ...
    let stream_throughput = (stream_success * BUFFER_SIZE) as f64 * 8.0 / stream_duration.as_secs_f64() / 1_000_000.0;
    ...
    let zero_copy_throughput = (zero_copy_success * BUFFER_SIZE) as f64 * 8.0 / zero_copy_duration.as_secs_f64() / 1_000_000.0;
    ...
    println!("   🔄 Stream handle:      {:?} ({:.2} Mbps) - {}/{} success", 
             stream_duration, stream_throughput, stream_success, ITERATIONS);
    println!("   ⚡ Zero-copy:          {:?} ({:.2} Mbps) - {}/{} success", 
             zero_copy_duration, zero_copy_throughput, zero_copy_success, ITERATIONS);
    
    let best_throughput = tell_throughput.max(stream_throughput).max(zero_copy_throughput);
    println!("   🏆 Best TCP throughput: {:.2} Mbps ({:.2} MB/s)", best_throughput, best_throughput / 8.0);
