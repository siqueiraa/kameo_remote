...
async fn test_direct_tell_performance(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing direct tell() with {} iterations of {}KB", ITERATIONS, BUFFER_SIZE / 1024);
    ...
    println!("   📤 Successful operations: {}/{}", successful_operations, ITERATIONS);
    ...
    println!("   ⚡ Avg per operation: {:.2} μs", elapsed.as_micros() as f64 / successful_operations as f64);
...
async fn test_ring_buffer_streaming_performance(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing ring buffer streaming with {} iterations of {}KB", ITERATIONS, BUFFER_SIZE / 1024);
    ...
    let submit_throughput = (successful_operations * BUFFER_SIZE) as f64 * 8.0 / write_elapsed.as_secs_f64() / 1_000_000.0;
    ...
    println!("   📤 Successful operations: {}/{}", successful_operations, ITERATIONS);
    println!("   📤 Total bytes written: {} ({:.2} MB)", total_bytes, total_bytes as f64 / 1_000_000.0);
    println!("   ⏱️  Write duration: {:?}", write_elapsed);
    println!("   ⏱️  Total duration: {:?}", total_elapsed);
    println!("   🚀 Submit rate: {:.2} Mbps ({:.2} MB/s) - how fast data enters ring buffer", submit_throughput, submit_throughput / 8.0);
    println!("   🚀 TCP throughput: {:.2} Mbps ({:.2} MB/s) - actual network transmission", actual_throughput, actual_throughput / 8.0);
    println!("   📊 Operations/sec: {:.2}", ops_per_second);
    println!("   ⚡ Avg submit latency: {:.2} μs", write_elapsed.as_micros() as f64 / successful_operations as f64);
...
async fn test_concurrent_tell_operations(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing {} concurrent tell() tasks with {} iterations each", CONCURRENT_TASKS, ITERATIONS);
    ...
    for task in tasks {
        ...
        println!("   🔄 Task {}: {}/{} successful", task_id, successful, ITERATIONS);
    ...
    println!("   📤 Total successful operations: {}/{}", total_successful, CONCURRENT_TASKS * ITERATIONS);
    println!("   📤 Total bytes: {} ({:.2} MB)", total_bytes, total_bytes as f64 / 1_000_000.0);
    println!("   ⏱️  Duration: {:?}", elapsed);
    println!("   🚀 Throughput: {:.2} Mbps ({:.2} MB/s)", throughput_mbps, throughput_mbps / 8.0);
    println!("   📊 Operations/sec: {:.2}", ops_per_second);
    println!("   ⚡ Avg per operation: {:.2} μs", elapsed.as_micros() as f64 / total_successful as f64);
...
async fn test_concurrent_ring_buffer_operations(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing {} concurrent ring buffer tasks with {} iterations each", CONCURRENT_TASKS, ITERATIONS);
    ...
    for task_id in 0..CONCURRENT_TASKS {
        ...
        let task = tokio::spawn(async move {
            ...
            (task_id, successful_operations, ring_buffer_handle.bytes_written())
        });
        tasks.push(task);
    }
    ...
    for task in tasks {
        ...
        println!("   🔄 Task {}: {}/{} successful, {} bytes written", task_id, successful, ITERATIONS, bytes_written);
        total_successful += successful;
    ...
    println!("   📤 Total successful operations: {}/{}", total_successful, CONCURRENT_TASKS * ITERATIONS);
    println!("   📤 Total bytes written: {} ({:.2} MB)", total_bytes_written, total_bytes_written as f64 / 1_000_000.0);
    println!("   ⏱️  Duration: {:?}", elapsed);
    println!("   🚀 Throughput: {:.2} Mbps ({:.2} MB/s)", throughput_mbps, throughput_mbps / 8.0);
    println!("   📊 Operations/sec: {:.2}", ops_per_second);
    println!("   ⚡ Avg per operation: {:.2} μs", elapsed.as_micros() as f64 / total_successful as f64);
...
async fn test_mixed_workload_comparison(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing mixed workload: tell() + ring buffer operations with {} iterations each", ITERATIONS);
    ...
    println!("   📤 Tell operations: {}/{} successful", tell_successful, ITERATIONS);
    println!("   📤 Ring buffer operations: {}/{} successful", ring_buffer_successful, ITERATIONS);
    println!("   📤 Total bytes: {} ({:.2} MB)", total_bytes, total_bytes as f64 / 1_000_000.0);
    println!("   ⏱️  Duration: {:?}", elapsed);
    println!("   🚀 Combined throughput: {:.2} Mbps ({:.2} MB/s)", throughput_mbps, throughput_mbps / 8.0);
    println!("   📊 Operations/sec: {:.2}", ops_per_second);
    println!("   ⚡ Avg per operation: {:.2} μs", elapsed.as_micros() as f64 / total_ops as f64);
    println!("   📊 Ring buffer: {} pending, {} available", pending, available);
