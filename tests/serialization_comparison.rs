...
pub enum RkyvRegistryMessage {
    DeltaGossip { delta: RkyvRegistryDelta },
    FullSync { 
    ...
    FullSyncRequest { sender_addr: String, sequence: u64, wall_clock_time: u64 },
    FullSyncResponse { 
        local_actors: Vec<(String, RkyvActorLocation)>,
        known_actors: Vec<(String, RkyvActorLocation)>,
        sender_addr: String,
    ...
    DeltaGossipResponse { delta: RkyvRegistryDelta },
}

/// Helper function to create test data for serialization benchmarks
fn create_test_delta_gossip(num_changes: usize) -> (registry::RegistryMessage, RkyvRegistryMessage) {
...
async fn test_serialization_performance_comparison() {
    ...
    for (num_changes, description) in test_sizes {
        ...
        println!("   - Serialization time: {:?} ({:.3} μs)", bincode_serialize_time, bincode_serialize_time.as_nanos() as f64 / 1000.0);
        println!("   - Serialized size: {} bytes", bincode_data.len());
        
        // Bincode deserialization test
        let bincode_deserialize_start = Instant::now();
        let _bincode_deserialized: registry::RegistryMessage = bincode::deserialize(&bincode_data).unwrap();
        let bincode_deserialize_time = bincode_deserialize_start.elapsed();
        
        println!("   - Deserialization time: {:?} ({:.3} μs)", bincode_deserialize_time, bincode_deserialize_time.as_nanos() as f64 / 1000.0);
        println!("   - Total time: {:?} ({:.3} μs)", bincode_serialize_time + bincode_deserialize_time, (bincode_serialize_time + bincode_deserialize_time).as_nanos() as f64 / 1000.0);
        ...
        println!("   - Serialization time: {:?} ({:.3} μs)", rkyv_serialize_time, rkyv_serialize_time.as_nanos() as f64 / 1000.0);
        println!("   - Serialized size: {} bytes", rkyv_data.len());
        
        // rkyv deserialization test (zero-copy)
        let rkyv_deserialize_start = Instant::now();
        let archived = rkyv::access::<ArchivedRkyvRegistryMessage, rkyv::rancor::Error>(&rkyv_data).unwrap();
        let rkyv_deserialize_time = rkyv_deserialize_start.elapsed();
        
        println!("   - Deserialization time: {:?} ({:.3} μs)", rkyv_deserialize_time, rkyv_deserialize_time.as_nanos() as f64 / 1000.0);
        println!("   - Total time: {:?} ({:.3} μs)", rkyv_serialize_time + rkyv_deserialize_time, (rkyv_serialize_time + rkyv_deserialize_time).as_nanos() as f64 / 1000.0);
        
        // Full deserialization test (for fair comparison)
        let rkyv_full_deserialize_start = Instant::now();
        let _rkyv_deserialized: RkyvRegistryMessage = rkyv::from_bytes::<_, rkyv::rancor::Error>(&rkyv_data).unwrap();
        let rkyv_full_deserialize_time = rkyv_full_deserialize_start.elapsed();
        
        println!("   - Full deserialization time: {:?} ({:.3} μs)", rkyv_full_deserialize_time, rkyv_full_deserialize_time.as_nanos() as f64 / 1000.0);
        println!("   - Total time (with full deser): {:?} ({:.3} μs)", rkyv_serialize_time + rkyv_full_deserialize_time, (rkyv_serialize_time + rkyv_full_deserialize_time).as_nanos() as f64 / 1000.0);
        ...
        let speed_ratio_zero_copy = bincode_total.as_nanos() as f64 / rkyv_total_zero_copy.as_nanos() as f64;
        let speed_ratio_full = bincode_total.as_nanos() as f64 / rkyv_total_full.as_nanos() as f64;
        
        println!("\n🎯 Performance Comparison:");
        println!("   - Size comparison: bincode {} bytes vs rkyv {} bytes ({:.2}x ratio)", bincode_data.len(), rkyv_data.len(), size_ratio);
        println!("   - Speed (zero-copy): rkyv is {:.2}x faster than bincode", speed_ratio_zero_copy);
        println!("   - Speed (full deser): rkyv is {:.2}x faster than bincode", speed_ratio_full);
        
        // Throughput calculation
        let bincode_throughput = bincode_data.len() as f64 / bincode_total.as_secs_f64() / (1024.0 * 1024.0);
        let rkyv_throughput_zero_copy = rkyv_data.len() as f64 / rkyv_total_zero_copy.as_secs_f64() / (1024.0 * 1024.0);
        let rkyv_throughput_full = rkyv_data.len() as f64 / rkyv_total_full.as_secs_f64() / (1024.0 * 1024.0);
        
        println!("   - Throughput bincode: {:.2} MB/s", bincode_throughput);
        println!("   - Throughput rkyv (zero-copy): {:.2} MB/s", rkyv_throughput_zero_copy);
        println!("   - Throughput rkyv (full deser): {:.2} MB/s", rkyv_throughput_full);
    }
    ...
    println!("   - Average per iteration: {:?} ({:.3} μs)", bulk_bincode_time / iterations, bulk_bincode_time.as_nanos() as f64 / iterations as f64 / 1000.0);
    ...
    for _ in 0..iterations {
        ...
        let archived = rkyv::access::<ArchivedRkyvRegistryMessage, rkyv::rancor::Error>(&data).unwrap();
    ...
    println!("   - Average per iteration: {:?} ({:.3} μs)", bulk_rkyv_time / iterations, bulk_rkyv_time.as_nanos() as f64 / iterations as f64 / 1000.0);
    
    let bulk_improvement = bulk_bincode_time.as_nanos() as f64 / bulk_rkyv_time.as_nanos() as f64;
    println!("   - rkyv is {:.2}x faster than bincode in bulk operations", bulk_improvement);
...
async fn test_full_sync_serialization_comparison() {
    ...
    for (num_actors, description) in test_sizes {
        ...
        let rkyv_deserialize_start = Instant::now();
        let archived = rkyv::access::<ArchivedRkyvRegistryMessage, rkyv::rancor::Error>(&rkyv_data).unwrap();
        let _: RkyvRegistryMessage = rkyv::from_bytes::<_, rkyv::rancor::Error>(&rkyv_data).unwrap();
        let rkyv_deserialize_time = rkyv_deserialize_start.elapsed();
        
        println!("   bincode: {} bytes, {:?} total time", bincode_data.len(), bincode_serialize_time + bincode_deserialize_time);
        println!("   rkyv: {} bytes, {:?} total time", rkyv_data.len(), rkyv_serialize_time + rkyv_deserialize_time);
        
        let improvement = (bincode_serialize_time + bincode_deserialize_time).as_nanos() as f64 / (rkyv_serialize_time + rkyv_deserialize_time).as_nanos() as f64;
        println!("   🎯 rkyv is {:.2}x faster for FullSync messages", improvement);
    }
