...
async fn test_mutual_authentication() {
    ...
    registry_a.register("test_actor".to_string(), "127.0.0.1:8000".parse().unwrap()).await
    ...
    assert_eq!(stats_a.active_peers, 1, "Registry A should have 1 connected peer");
    assert_eq!(stats_b.active_peers, 1, "Registry B should have 1 connected peer");
}
...
async fn test_impersonation_prevention() {
    ...
    registry_a.registry.add_peer_with_node_id(addr_imposter, Some(node_id_a)).await; // Wrong NodeId!
    registry_imposter.registry.add_peer_with_node_id(addr_a, Some(node_id_imposter)).await;

    // Register an actor on imposter
    registry_imposter.register("secret_actor".to_string(), "127.0.0.1:9000".parse().unwrap()).await
        .expect("Failed to register actor");
    ...
    assert!(location.is_none(), "Actor from imposter should not be found");

    // Stats should show failed connection
    let stats_a = registry_a.registry.get_stats().await;
    assert_eq!(stats_a.active_peers, 0, "Should have no connected peers due to NodeId mismatch");
}
...
async fn test_bidirectional_tls_communication() {
    ...
    registry_a.register("actor_a".to_string(), "127.0.0.1:10001".parse().unwrap()).await
        .expect("Failed to register actor A");
    
    registry_b.register("actor_b".to_string(), "127.0.0.1:10002".parse().unwrap()).await
...
async fn test_multi_node_tls_chain() {
    ...
    registry_a.registry.add_peer_with_node_id(addr_b, Some(node_id_b)).await;
    registry_b.registry.add_peer_with_node_id(addr_c, Some(node_id_c)).await;

    // Register actors on each node
    registry_a.register("actor_a".to_string(), "127.0.0.1:11001".parse().unwrap()).await.unwrap();
    registry_b.register("actor_b".to_string(), "127.0.0.1:11002".parse().unwrap()).await.unwrap();
    registry_c.register("actor_c".to_string(), "127.0.0.1:11003".parse().unwrap()).await.unwrap();
...
async fn test_tls_reconnection() {
    ...
    // Register an actor on A
    registry_a.register("persistent_actor".to_string(), "127.0.0.1:12000".parse().unwrap()).await.unwrap();
    ...
    assert_eq!(stats.active_peers, 0, "A should have no connected peers after B shutdown");
    ...
    assert!(location.is_some(), "B should know about A's actor after reconnection");
...
async fn test_node_id_dns_encoding() {
    ...
    for _ in 0..10 {
        ...
        assert_eq!(decoded.unwrap(), node_id, "Round-trip should preserve NodeId");
    }
...
async fn test_certificate_generation() {
    ...
    for _ in 0..5 {
        ...
        let tls_config = kameo_remote::tls::TlsConfig::new(secret_key)
            .expect("Failed to create TLS config");
        
        // Verify the node_id matches
        assert_eq!(tls_config.node_id, node_id, "TLS config should have correct NodeId");
