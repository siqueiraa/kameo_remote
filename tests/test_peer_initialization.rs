...
async fn test_peer_initialization_with_names() {
    ...
    // Register actors
    handle1.register("service1".to_string(), "127.0.0.1:46001".parse().unwrap()).await.unwrap();
    handle2.register("service2".to_string(), "127.0.0.1:46002".parse().unwrap()).await.unwrap();
    handle3.register("service3".to_string(), "127.0.0.1:46003".parse().unwrap()).await.unwrap();
    ...
    for (handle, node_name) in [(&handle1, "node1"), (&handle2, "node2"), (&handle3, "node3")] {
        println!("\nFrom {}:", node_name);
        for service in ["service1", "service2", "service3"] {
            let result = handle.lookup(service).await;
            println!("  lookup('{}') = {:?}", service, result.is_some());
            assert!(result.is_some(), "{} should discover {}", node_name, service);
        }
    }
