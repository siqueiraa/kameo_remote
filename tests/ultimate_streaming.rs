...
async fn test_ultimate_no_ack_throughput(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing no-ack mode with {}KB buffers, {} iterations", 
    ...
    let throughput_gbps = (total_bytes * 8.0) / (elapsed.as_nanos() as f64 / 1_000_000_000.0) / 1_000_000_000.0;
    
    println!("📈 Ultimate No-Ack Throughput Results:");
    println!("   📤 Bytes Written: {} bytes ({:.2} MB)", ultimate.bytes_written(), total_bytes / 1_000_000.0);
    println!("   ⏱️  Duration: {:?}", elapsed);
    println!("   🚀 Throughput: {:.2} Gbps ({:.2} MB/s)", throughput_gbps, throughput_gbps * 1000.0 / 8.0);
    println!("   ⚡ Avg per write: {:.2} ns", elapsed.as_nanos() as f64 / ITERATIONS as f64);
    println!("   📊 Success Rate: {:.1}%", (total_bytes / (BUFFER_SIZE * ITERATIONS) as f64) * 100.0);
...
async fn test_ultimate_vectored_no_ack(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing ultimate vectored no-ack with {} buffers of {}KB each", 
             VECTORS_PER_WRITE, BUFFER_SIZE / 1024);
    ...
    println!("   ⏱️  Duration: {:?}", elapsed);
    println!("   🚀 Throughput: {:.2} Gbps ({:.2} MB/s)", throughput_gbps, throughput_gbps * 1000.0 / 8.0);
    println!("   ⚡ Avg per vectored write: {:.2} μs", elapsed.as_micros() as f64 / ITERATIONS as f64);
}
...
async fn test_ultimate_bulk_no_ack(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing ultimate bulk no-ack with {} chunks of {}KB each", 
             CHUNKS_PER_BULK, CHUNK_SIZE / 1024);
    ...
    println!("   ⏱️  Duration: {:?}", elapsed);
    println!("   🚀 Throughput: {:.2} Gbps ({:.2} MB/s)", throughput_gbps, throughput_gbps * 1000.0 / 8.0);
    println!("   ⚡ Avg per bulk write: {:.2} μs", elapsed.as_micros() as f64 / ITERATIONS as f64);
}
...
async fn test_ultimate_continuous_streaming(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing ultimate continuous streaming for {:?} with {}KB buffers", 
             STREAM_DURATION, BUFFER_SIZE / 1024);
    ...
    println!("   📤 Bytes Written: {} bytes ({:.2} GB)", ultimate.bytes_written(), total_bytes / 1_000_000_000.0);
    println!("   🔄 Iterations: {}", iteration_count);
    println!("   ⏱️  Duration: {:?}", elapsed);
    println!("   🚀 Sustained Throughput: {:.2} Gbps ({:.2} MB/s)", throughput_gbps, throughput_gbps * 1000.0 / 8.0);
    println!("   📊 Messages/sec: {:.2}", messages_per_second);
    println!("   ⚡ Avg per message: {:.2} ns", elapsed.as_nanos() as f64 / iteration_count as f64);
    println!("   📊 Success Rate: {:.1}%", (total_bytes / (BUFFER_SIZE * iteration_count) as f64) * 100.0);
...
async fn test_ultimate_vs_zero_copy(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    let zero_copy_throughput = (total_bytes * 8.0) / (zero_copy_duration.as_nanos() as f64 / 1_000_000_000.0) / 1_000_000_000.0;
    let ultimate_throughput = (ultimate.bytes_written() as f64 * 8.0) / (ultimate_duration.as_nanos() as f64 / 1_000_000_000.0) / 1_000_000_000.0;
    let ultimate_vectored_throughput = (ultimate.bytes_written() as f64 * 8.0) / (ultimate_vectored_duration.as_nanos() as f64 / 1_000_000_000.0) / 1_000_000_000.0;
    
    println!("\n📊 Ultimate vs Zero-Copy Results:");
    println!("   📤 Zero-Copy:           {:?} ({:.2} Gbps)", zero_copy_duration, zero_copy_throughput);
    println!("   🚀 Ultimate No-Ack:     {:?} ({:.2} Gbps)", ultimate_duration, ultimate_throughput);
    println!("   ⚡ Ultimate Vectored:    {:?} ({:.2} Gbps)", ultimate_vectored_duration, ultimate_vectored_throughput);
...
async fn test_ultimate_streaming_basic() {
    ...
    println!("📤 Testing ultimate no-ack write: {:?}", std::str::from_utf8(test_data));
    ...
    // Test vectored write
    let bufs = vec![b"Hello, ".as_slice(), b"ultimate ".as_slice(), b"vectored!".as_slice()];
    let result = ultimate.write_vectored_no_ack(&bufs).await;
    assert!(result.is_ok(), "Ultimate vectored write failed: {:?}", result.err());
    ...
    assert!(result.is_ok(), "Ultimate bulk write failed: {:?}", result.err());
