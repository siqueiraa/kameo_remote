...
async fn profile_gossip_latency_components() {
    ...
    println!("📊 Serialization: {:?} ({} bytes)", serialization_time, serialized.len());
    ...
    println!("📊 Message creation: {:?} ({} bytes)", message_creation_time, message.len());
    ...
    let total_overhead = serialization_time + deserialization_time + message_creation_time + channel_send_time;
    println!("   - Total measured overhead: {:?}", total_overhead);
    println!("   - Unaccounted time: {:?}", e2e_time.saturating_sub(total_overhead));
    ...
    assert!(e2e_time < Duration::from_millis(10), "End-to-end latency should be <10ms");
}