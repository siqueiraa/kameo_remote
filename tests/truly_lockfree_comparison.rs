...
async fn test_traditional_mutex_tell(conn: &ConnectionHandle) {
    ...
    println!("📊 Testing traditional mutex-based tell() with {} iterations", ITERATIONS);
...
async fn test_truly_lockfree_tell(conn: &mut ConnectionHandle) {
    ...
    println!("📊 Testing truly lock-free tell() with {} iterations", ITERATIONS);
    ...
    println!("   📤 Successful operations: {}/{}", successful_operations, ITERATIONS);
    println!("   📤 Total bytes submitted: {} ({:.2} MB)", total_bytes, total_bytes as f64 / 1_000_000.0);
    println!("   📤 Total bytes written: {} ({:.2} MB)", actual_bytes, actual_bytes as f64 / 1_000_000.0);
    println!("   ⏱️  Submit duration: {:?}", submit_elapsed);
    println!("   ⏱️  Total duration: {:?}", total_elapsed);
    println!("   🚀 Submit throughput: {:.2} Mbps ({:.2} MB/s)", submit_throughput, submit_throughput / 8.0);
    println!("   🚀 TCP throughput: {:.2} Mbps ({:.2} MB/s)", actual_throughput, actual_throughput / 8.0);
    println!("   📊 Operations/sec: {:.2}", ops_per_second);
    println!("   ⚡ Avg submit latency: {:.2} μs", submit_elapsed.as_micros() as f64 / successful_operations as f64);
...
async fn test_mixed_workload_lockfree(conn: &mut ConnectionHandle) {
    ...
    println!("📊 Testing mixed workload - all operations lock-free with {} iterations", ITERATIONS);
    ...
    let actual_bytes = ring_buffer.bytes_written();
    let submit_throughput = (total_bytes_submitted as f64 * 8.0) / submit_elapsed.as_secs_f64() / 1_000_000.0;
    let actual_throughput = actual_bytes as f64 * 8.0 / total_elapsed.as_secs_f64() / 1_000_000.0;
    let (pending, available) = ring_buffer.buffer_status();
    
    println!("📈 Mixed Workload Lock-Free Results:");
    println!("   📤 Tell operations: {}/{} successful", tell_successful, ITERATIONS);
    println!("   📤 Stream operations: {}/{} successful", stream_successful, ITERATIONS);
    println!("   📤 Total operations: {}", total_operations);
    println!("   📤 Total bytes submitted: {} ({:.2} MB)", total_bytes_submitted, total_bytes_submitted as f64 / 1_000_000.0);
    println!("   📤 Total bytes written: {} ({:.2} MB)", actual_bytes, actual_bytes as f64 / 1_000_000.0);
    println!("   ⏱️  Submit duration: {:?}", submit_elapsed);
    println!("   ⏱️  Total duration: {:?}", total_elapsed);
    println!("   🚀 Submit throughput: {:.2} Mbps ({:.2} MB/s)", submit_throughput, submit_throughput / 8.0);
...
async fn test_concurrent_all_lockfree(conn: &mut ConnectionHandle) {
    ...
    println!("📊 Testing {} concurrent lock-free tasks with {} iterations each", CONCURRENT_TASKS, ITERATIONS);
...
async fn test_high_throughput_comparison(conn: &mut ConnectionHandle) {
    ...
    println!("📊 Testing high-throughput lock-free operations for {:?}", STREAM_DURATION);
    ...
    let submit_throughput = (submitted_bytes as f64 * 8.0) / submit_elapsed.as_secs_f64() / 1_000_000.0;
    let actual_throughput = actual_bytes as f64 * 8.0 / total_elapsed.as_secs_f64() / 1_000_000.0;
    ...
    println!("   📤 Total bytes submitted: {} ({:.2} MB)", submitted_bytes, submitted_bytes as f64 / 1_000_000.0);
    println!("   📤 Total bytes written: {} ({:.2} MB)", actual_bytes, actual_bytes as f64 / 1_000_000.0);
    println!("   ⏱️  Submit duration: {:?}", submit_elapsed);
    println!("   ⏱️  Total duration: {:?}", total_elapsed);
    println!("   🚀 Submit throughput: {:.2} Mbps ({:.2} MB/s)", submit_throughput, submit_throughput / 8.0);
    println!("   🚀 TCP throughput: {:.2} Mbps ({:.2} MB/s)", actual_throughput, actual_throughput / 8.0);
    println!("   📊 Operations/sec: {:.2}", ops_per_second);
    println!("   ⚡ Avg per operation: {:.2} ns", submit_elapsed.as_nanos() as f64 / iterations as f64);
    println!("   📊 Ring buffer: {} pending, {} available", pending, available);
    ...
    if actual_throughput > 1000.0 {
        ...
        println!("   ℹ️  Lock-free TCP throughput: {:.2} Mbps", actual_throughput);
    }
}