...
fn get_lock_free_stream_helper(conn: &crate::connection_pool::ConnectionHandle, channel_id: ChannelId) -> LockFreeStreamHandle {
...
async fn test_lock_free_streaming() {
    ...
    println!("✅ Connection established: {} -> {}", node1_addr, node2_addr);
...
async fn test_basic_lock_free_streaming(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📤 Testing lock-free stream write: {:?}", std::str::from_utf8(test_data));
...
async fn test_concurrent_lock_free_tell(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing concurrent ring buffer + tell with {} iterations", ITERATIONS);
    ...
    println!("   📤 Ring buffer operations: {}/{} successful", stream_success, ITERATIONS);
    println!("   📤 Tell operations: {}/{} successful", tell_success, ITERATIONS);
    println!("   📤 Total bytes: {} bytes ({:.2} MB)", total_bytes, total_bytes as f64 / 1_000_000.0);
    println!("   ⏱️  Duration: {:?}", elapsed);
    println!("   🚀 Combined throughput: {:.2} Mbps ({:.2} MB/s)", throughput_mbps, throughput_mbps / 8.0);
    println!("   📊 Ring buffer: {} pending, {} available", pending, available);
...
async fn test_multiple_lock_free_streams(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing multiple ring buffer streams with {} iterations each", ITERATIONS);
    ...
    println!("   📤 Stream1 bytes: {} ({:.2} MB)", bytes1, bytes1 as f64 / 1_000_000.0);
    println!("   📤 Stream2 bytes: {} ({:.2} MB)", bytes2, bytes2 as f64 / 1_000_000.0);
    println!("   📤 Stream3 bytes: {} ({:.2} MB)", bytes3, bytes3 as f64 / 1_000_000.0);
    println!("   📤 Bulk bytes: {} ({:.2} MB)", bulk_bytes, bulk_bytes as f64 / 1_000_000.0);
...
async fn test_high_throughput_lock_free(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing ring buffer sustained throughput for {:?}", STREAM_DURATION);
    ...
    println!("   📤 Total bytes: {} ({:.2} GB)", lock_free_handle.bytes_written(), total_bytes / 1_000_000_000.0);
    println!("   🔄 Iterations: {}", iterations);
    println!("   ⏱️  Duration: {:?}", elapsed);
    println!("   🚀 Throughput: {:.2} Mbps ({:.2} MB/s)", throughput_mbps, throughput_mbps / 8.0);
    println!("   📊 Writes/sec: {:.2}", writes_per_second);
    println!("   ⚡ Avg per write: {:.2} ns", elapsed.as_nanos() as f64 / iterations as f64);
    println!("   📊 Ring buffer: {} pending, {} available", pending, available);
    ...
    if throughput_mbps > 20000.0 {
        ...
        println!("   ℹ️  Ring buffer performance: {:.2} Mbps", throughput_mbps);
    }
...
async fn test_ring_buffer_performance(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing ring buffer burst performance with {} iterations", ITERATIONS);
    ...
    let total_bytes = lock_free_handle.bytes_written() as f64;
    let write_throughput = (successful_writes * BUFFER_SIZE) as f64 * 8.0 / write_elapsed.as_secs_f64() / 1_000_000.0;
    let actual_throughput = total_bytes * 8.0 / total_elapsed.as_secs_f64() / 1_000_000.0;
    
    let (pending, available) = lock_free_handle.buffer_status();
    
    println!("📈 Ring Buffer Burst Performance Results:");
    println!("   📤 Write operations: {}/{} successful", successful_writes, ITERATIONS);
    println!("   📤 Buffer full events: {}", buffer_full_count);
    println!("   📤 Total bytes written: {} ({:.2} MB)", lock_free_handle.bytes_written(), total_bytes / 1_000_000.0);
    println!("   ⏱️  Write duration: {:?}", write_elapsed);
    println!("   ⏱️  Total duration: {:?}", total_elapsed);
    println!("   🚀 Write throughput: {:.2} Mbps ({:.2} MB/s)", write_throughput, write_throughput / 8.0);
    println!("   🚀 Actual throughput: {:.2} Mbps ({:.2} MB/s)", actual_throughput, actual_throughput / 8.0);
    println!("   📊 Ring buffer: {} pending, {} available", pending, available);
    println!("   ⚡ Avg write latency: {:.2} ns", write_elapsed.as_nanos() as f64 / successful_writes as f64);
...
async fn test_lock_free_basic_functionality() {
    ...
    println!("✅ Created lock-free stream: {:?}", lock_free_handle.channel_id());
