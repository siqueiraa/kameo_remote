...
async fn test_tell_with_lookup_and_performance_comparison() {
    ...
    peer2.connect(&"127.0.0.1:29002".parse().unwrap()).await.unwrap();
    peer3.connect(&"127.0.0.1:29003".parse().unwrap()).await.unwrap();
    ...
    peer1_from_2.connect(&"127.0.0.1:29001".parse().unwrap()).await.unwrap();
    peer3_from_2.connect(&"127.0.0.1:29003".parse().unwrap()).await.unwrap();
    ...
    peer1_from_3.connect(&"127.0.0.1:29001".parse().unwrap()).await.unwrap();
    peer2_from_3.connect(&"127.0.0.1:29002".parse().unwrap()).await.unwrap();
    ...
    println!("✅ All actors propagated in: {:?} ({:.3} ms)", propagation_time, propagation_time.as_millis() as f64);
    ...
    let chat_location = node1.lookup("chat_service").await
        .expect("chat_service not found");
    let chat_conn = node1.get_connection(match chat_location.peer_id.as_str() {
    ...
    let auth_location = node1.lookup("auth_service").await
        .expect("auth_service not found");
    let auth_conn = node1.get_connection(match auth_location.peer_id.as_str() {
        "node1" => "127.0.0.1:29001".parse().unwrap(),
        "node2" => "127.0.0.1:29002".parse().unwrap(),
        "node3" => "127.0.0.1:29003".parse().unwrap(),
        _ => panic!("Unknown peer"),
    ...
    let storage_location = node1.lookup("storage_service").await
        .expect("storage_service not found");
    let storage_conn = node1.get_connection(match storage_location.peer_id.as_str() {
        "node1" => "127.0.0.1:29001".parse().unwrap(),
        "node2" => "127.0.0.1:29002".parse().unwrap(),
        "node3" => "127.0.0.1:29003".parse().unwrap(),
        _ => panic!("Unknown peer"),
    ...
    println!("     - chat_service at {} (peer: {})", chat_location.address, chat_location.peer_id);
    println!("     - auth_service at {} (peer: {})", auth_location.address, auth_location.peer_id);
    println!("     - storage_service at {} (peer: {})", storage_location.address, storage_location.peer_id);
    println!("   📈 Total lookup and connection time: {:?} ({:.3} μs)", 
            total_lookup_time, total_lookup_time.as_nanos() as f64 / 1000.0);
    ...
    let chat_ref = ActorRef { name: "chat_service", location: chat_location, conn: chat_conn };
    let auth_ref = ActorRef { name: "auth_service", location: auth_location, conn: auth_conn };
    let storage_ref = ActorRef { name: "storage_service", location: storage_location, conn: storage_conn };
    ...
    for (i, (actor, msg)) in test_messages.iter().enumerate() {
        println!("     {}. {} <- {:?} ({} bytes)", i+1, actor, 
                std::str::from_utf8(msg).unwrap(), msg.len());
    }
    ...
    for (i, (actor_name, message)) in test_messages.iter().enumerate() {
        ...
        println!("     Message {} to {}: {:?} ({:.3} μs)",
                i+1, actor_ref.name, send_time, send_time.as_nanos() as f64 / 1000.0);
    }
    ...
    println!("     - Total time: {:?} ({:.3} μs)", individual_total, individual_total.as_nanos() as f64 / 1000.0);
    println!("     - Average per message: {:?} ({:.3} μs)", individual_avg, individual_avg.as_nanos() as f64 / 1000.0);
    ...
    println!("     - Total time: {:?} ({:.3} μs)", sequential_total, sequential_total.as_nanos() as f64 / 1000.0);
    println!("     - Average per message: {:?} ({:.3} μs)", sequential_avg, sequential_avg.as_nanos() as f64 / 1000.0);
    ...
    let chat_messages: Vec<&[u8]> = test_messages.iter()
        .filter(|(actor, _)| *actor == "chat_service")
        .map(|(_, msg)| *msg)
        .collect();
    
    let auth_messages: Vec<&[u8]> = test_messages.iter()
        .filter(|(actor, _)| *actor == "auth_service")
        .map(|(_, msg)| *msg)
        .collect();
    
    let storage_messages: Vec<&[u8]> = test_messages.iter()
    ...
    println!("     - storage_service: {} messages", storage_messages.len());
    
    // Send batches using actor refs
    chat_ref.conn.tell(kameo_remote::connection_pool::TellMessage::batch(chat_messages)).await.unwrap();
    auth_ref.conn.tell(kameo_remote::connection_pool::TellMessage::batch(auth_messages)).await.unwrap();
    storage_ref.conn.tell(kameo_remote::connection_pool::TellMessage::batch(storage_messages)).await.unwrap();
    
    let batch_total = batch_start.elapsed();
    let batch_avg = batch_total / test_messages.len() as u32;
    
    println!("   📈 True Batch Results:");
    println!("     - Total time: {:?} ({:.3} ms)", batch_total, batch_total.as_millis() as f64);
    println!("     - Average per message: {:?} ({:.3} μs)", batch_avg, batch_avg.as_nanos() as f64 / 1000.0);
    ...
    println!("     - Total: {:?} ({:.3} μs)", individual_total, individual_total.as_nanos() as f64 / 1000.0);
    println!("     - Per message: {:?} ({:.3} μs)", individual_avg, individual_avg.as_nanos() as f64 / 1000.0);
    
    println!("   Sequential (same as individual, just timed differently):");
    println!("     - Total: {:?} ({:.3} μs)", sequential_total, sequential_total.as_nanos() as f64 / 1000.0);
    println!("     - Per message: {:?} ({:.3} μs)", sequential_avg, sequential_avg.as_nanos() as f64 / 1000.0);
    
    println!("   Batch (group by destination):");
    println!("     - Total: {:?} ({:.3} μs)", batch_total, batch_total.as_nanos() as f64 / 1000.0);
    println!("     - Per message: {:?} ({:.3} μs)", batch_avg, batch_avg.as_nanos() as f64 / 1000.0);
    ...
    println!("   2. Batch sending provides {:.1}x-{:.1}x speedup by reducing network overhead", 
            batch_vs_sequential, batch_vs_individual);
    println!("   3. Initial lookup + connection time: {:.3} μs per actor", 
            total_lookup_time.as_nanos() as f64 / 3000.0);
    println!("   4. Once actor refs are created, sending is very fast (~{:.1} μs per message)", 
    ...
    println!("\n🔸 Test 3A: High Volume Individual ({}x3 = {} total messages)", volume_count, volume_count * 3);
    ...
    println!("     - Total: {:?} ({:.3} ms)", vol_individual_total, vol_individual_total.as_millis() as f64);
    println!("     - Per message: {:?} ({:.3} μs)", vol_individual_per_msg, vol_individual_per_msg.as_nanos() as f64 / 1000.0);
    println!("     - Messages/sec: {:.0}", (volume_count * 3) as f64 / vol_individual_total.as_secs_f64());
    ...
    chat_ref.conn.tell(kameo_remote::connection_pool::TellMessage::batch(chat_batch_refs)).await.unwrap();
    auth_ref.conn.tell(kameo_remote::connection_pool::TellMessage::batch(auth_batch_refs)).await.unwrap();
    storage_ref.conn.tell(kameo_remote::connection_pool::TellMessage::batch(storage_batch_refs)).await.unwrap();
    ...
    println!("     - Total: {:?} ({:.3} ms)", vol_batch_total, vol_batch_total.as_millis() as f64);
    println!("     - Per message: {:?} ({:.3} μs)", vol_batch_per_msg, vol_batch_per_msg.as_nanos() as f64 / 1000.0);
    println!("     - Messages/sec: {:.0}", (volume_count * 3) as f64 / vol_batch_total.as_secs_f64());
    
    let vol_improvement = vol_individual_total.as_nanos() as f64 / vol_batch_total.as_nanos() as f64;
    println!("   🎯 High volume improvement: {:.1}x faster with batching", vol_improvement);
    ...
    println!("   4. For high-throughput scenarios, batching provides {:.0}x-{:.0}x improvement", 
            batch_vs_individual.min(vol_improvement), batch_vs_individual.max(vol_improvement));
...
async fn test_lookup_error_handling() {
    ...
    assert!(result.is_none(), "Lookup should return None for non-existent actors");
