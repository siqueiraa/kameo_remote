...
use kameo_remote::{
    GossipConfig, GossipRegistryHandle, RegistrationPriority,
};
...
async fn test_direct_tcp_stream_throughput() {
    ...
    let receiver_handle = GossipRegistryHandle::new(receiver_addr, vec![], Some(config.clone())).await.unwrap();
    let sender_handle = GossipRegistryHandle::new(sender_addr, vec![], Some(config.clone())).await.unwrap();
    
    // Register a test actor on receiver
    let actor_addr = "127.0.0.1:20003".parse().unwrap();
    
    receiver_handle.register_urgent(
    ...
    println!("🔗 Created direct TcpStream connection to: {}", actor_location.address);
    ...
    for chunk in test_data.chunks(CHUNK_SIZE) {
        ...
        while bytes_received < chunk.len() {
            let bytes_read = stream.read(&mut response_buffer[bytes_received..]).await.unwrap();
            if bytes_read == 0 {
...
async fn test_single_gossip_message_performance() {
    ...
    println!("   - Message content: {:?}", std::str::from_utf8(test_message).unwrap_or("binary"));
    ...
    println!("   - Transmission time (μs): {:.3}", single_msg_time.as_nanos() as f64 / 1000.0);
    ...
    println!("   - 10 messages sent in (μs): {:.3}", sequential_time.as_nanos() as f64 / 1000.0);
    println!("   - Average per message: {:?}", sequential_time / 10);
    println!("   - Average per message (μs): {:.3}", sequential_time.as_nanos() as f64 / 10000.0);
    ...
    assert!(single_msg_time < Duration::from_millis(1), "Single message should take <1ms on localhost");
    assert!(sequential_time < Duration::from_millis(10), "10 sequential messages should take <10ms on localhost");
}
...
async fn test_ask_performance() {
    ...
    // REUSE existing connection from pool (this is the key requirement)
    let connection_handle = sender_handle.get_connection(receiver_addr).await.unwrap();
    println!("🔗 Reusing existing gossip connection for ask() to: {}", connection_handle.addr);
    ...
    println!("📊 Sending {} request-response pairs via ask() using reused connection", MESSAGE_COUNT / 10);
    ...
    for i in 0..(MESSAGE_COUNT / 10) {
        ...
        assert_eq!(String::from_utf8_lossy(&response), expected_response, "Response should match expected format");
...
async fn test_gossip_propagation_performance() {
    ...
    let node1 = GossipRegistryHandle::new(node1_addr, vec![], Some(config.clone())).await.unwrap();
    let node2 = GossipRegistryHandle::new(node2_addr, vec![], Some(config.clone())).await.unwrap();
    let node3 = GossipRegistryHandle::new(node3_addr, vec![], Some(config.clone())).await.unwrap();
    ...
    println!("⏱️  REGISTRATION TIME: {:?} - {} actors registered", registration_time, actor_count);
    ...
    while !all_propagated && propagation_start.elapsed() < timeout {
        ...
        if propagation_start.elapsed().as_millis() % 1000 == 0 {
            println!("   - Propagated: {}/{} actors", propagated_count, actor_count);
        }
    }
    ...
    println!("   - Network efficiency: {:.2}% (3-node mesh)", (propagation_rate / actors_per_sec) * 100.0);
    ...
    println!("   - Final propagation: {}/{} actors", final_propagated, actor_count);
    ...
    assert!(final_propagated >= actor_count * 90 / 100, "Should propagate at least 90% of actors");
    assert!(propagation_time < Duration::from_secs(5), "Propagation should complete within 5 seconds");
...
async fn test_tell_performance() {
    ...
    println!("   - Total data: {:.2} MB", (test_message.len() * iterations) as f64 / (1024.0 * 1024.0));
    ...
    println!("\n📊 Testing SINGLE: {} individual tell() calls with automatic detection", iterations);
    ...
    let single_throughput = (1024 * iterations) as f64 / (1024.0 * 1024.0) / single_duration.as_secs_f64();
    
    println!("   - Total time: {:?}", single_duration);
    println!("   - Total time (μs): {:.3}", single_duration.as_nanos() as f64 / 1000.0);
    println!("   - Messages/sec: {:.0}", single_messages_per_sec);
    println!("   - Average latency: {:?}", single_avg_latency);
    println!("   - Average latency (μs): {:.3}", single_avg_latency.as_nanos() as f64 / 1000.0);
    println!("   - Throughput: {:.2} MB/s", single_throughput);
    
    // Test explicit TellMessage::single() 
    println!("\n📊 Testing EXPLICIT SINGLE: {} TellMessage::single() calls", iterations);
    
    let explicit_start = Instant::now();
    
    for _ in 0..iterations {
        connection_handle.tell(kameo_remote::connection_pool::TellMessage::single(&test_message)).await.unwrap();
    }
    ...
    println!("\n📊 Testing BATCH: Single TellMessage::batch() with {} messages", iterations);
    
    let batch_messages: Vec<&[u8]> = (0..iterations).map(|_| test_message.as_slice()).collect();
    
    println!("   - Batch configuration: {} messages of {} bytes each", batch_messages.len(), test_message.len());
    println!("   - Batch total size: {:.2} MB", (test_message.len() * iterations) as f64 / (1024.0 * 1024.0));
    println!("   - Batch method: TellMessage::batch() with Vec<&[u8]>");
    
    let batch_start = Instant::now();
    connection_handle.tell(kameo_remote::connection_pool::TellMessage::batch(batch_messages)).await.unwrap();
    ...
    let batch_throughput = (1024 * iterations) as f64 / (1024.0 * 1024.0) / batch_duration.as_secs_f64();
    
    println!("   - Total time: {:?}", batch_duration);
    println!("   - Total time (μs): {:.3}", batch_duration.as_nanos() as f64 / 1000.0);
    println!("   - Messages/sec: {:.0}", batch_messages_per_sec);
    println!("   - Average latency: {:?}", batch_avg_latency);
    println!("   - Average latency (μs): {:.3}", batch_avg_latency.as_nanos() as f64 / 1000.0);
    ...
    let latency_improvement = single_avg_latency.as_nanos() as f64 / batch_avg_latency.as_nanos() as f64;
    
    println!("\n📈 PERFORMANCE COMPARISON:");
    println!("   - Batch speedup: {:.2}x faster", speedup);
    println!("   - Latency improvement: {:.2}x better", latency_improvement);
    println!("   - Throughput improvement: {:.2}x better", batch_throughput / single_throughput);
    ...
    assert!(single_messages_per_sec > 1000.0, "Single should handle >1k messages/sec");
    assert!(batch_messages_per_sec > 5000.0, "Batch should handle >5k messages/sec");
    assert!(speedup > 2.0, "Batch should be at least 2x faster than single");
}
...
async fn test_seamless_batching_api() {
    ...
    let start = Instant::now();
    connection_handle.tell(kameo_remote::connection_pool::TellMessage::batch(message_refs)).await.unwrap();
    ...
    println!("   - Batch speedup: {:.2}x", single_total.as_nanos() as f64 / batch_total.as_nanos() as f64);
    
    // Cleanup
    receiver_handle.shutdown().await;
    sender_handle.shutdown().await;
    
    assert!(batch_total < single_total, "Batching should be faster than individual sends");
}
