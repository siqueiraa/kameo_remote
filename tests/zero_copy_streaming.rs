...
async fn test_zero_copy_max_throughput(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing with {}KB buffers, {}MB batch size, {} iterations", 
             BUFFER_SIZE / 1024, BATCH_SIZE / 1024 / 1024, ITERATIONS);
    ...
    let throughput_gbps = (total_bytes as f64 * 8.0) / (elapsed.as_nanos() as f64 / 1_000_000_000.0) / 1_000_000_000.0;
    ...
    println!("   🚀 Throughput: {:.2} Gbps ({:.2} MB/s)", throughput_gbps, throughput_mbps / 8.0);
    ...
    if throughput_gbps > 5.0 {
        ...
        println!("   ⚠️  Below line-rate target ({:.2} Gbps)", throughput_gbps);
    }
}
...
async fn test_zero_copy_ultra_low_latency(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing ultra-low latency with {}B buffers, {} iterations", BUFFER_SIZE, ITERATIONS);
    ...
    println!("   🏃 Min Latency: {:?} ({:.2} ns)", min_latency, min_latency.as_nanos());
    println!("   📊 Avg Latency: {:?} ({:.2} ns)", avg_latency, avg_latency.as_nanos());
    println!("   🐌 Max Latency: {:?} ({:.2} ns)", max_latency, max_latency.as_nanos());
...
async fn test_zero_copy_vectored_io(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing vectored I/O with {} buffers of {}KB each per write", 
...
async fn test_zero_copy_batch_processing(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing batch processing with {}KB buffers, {}KB batch size", 
             BUFFER_SIZE / 1024, BATCH_SIZE / 1024);
    ...
    println!("   ⏱️  Duration: {:?}", elapsed);
    println!("   🚀 Throughput: {:.2} Gbps ({:.2} MB/s)", throughput_gbps, throughput_gbps * 1000.0 / 8.0);
    println!("   🔄 Flush Count: {} (efficiency: {:.2}%)", flush_count, 100.0 * total_bytes as f64 / (flush_count as f64 * BATCH_SIZE as f64));
}
...
async fn test_zero_copy_continuous_streaming(conn: &crate::connection_pool::ConnectionHandle) {
    ...
    println!("📊 Testing continuous streaming for {:?} with {}KB buffers", STREAM_DURATION, BUFFER_SIZE / 1024);
    ...
    println!("   📤 Total Bytes: {} bytes ({:.2} GB)", total_bytes, total_bytes as f64 / 1_000_000_000.0);
...
async fn test_zero_copy_vs_regular_comparison() {
    ...
    let regular_throughput = (total_bytes * 8.0) / (regular_duration.as_nanos() as f64 / 1_000_000_000.0) / 1_000_000_000.0;
    let zero_copy_throughput = (total_bytes * 8.0) / (zero_copy_duration.as_nanos() as f64 / 1_000_000_000.0) / 1_000_000_000.0;
    let vectored_throughput = (total_bytes * 8.0) / (vectored_duration.as_nanos() as f64 / 1_000_000_000.0) / 1_000_000_000.0;
    
    println!("\n📊 Performance Comparison Results:");
    println!("   📤 Regular Stream:    {:?} ({:.2} Gbps)", regular_duration, regular_throughput);
    println!("   🚀 Zero-Copy:         {:?} ({:.2} Gbps)", zero_copy_duration, zero_copy_throughput);
    println!("   ⚡ Zero-Copy Vectored: {:?} ({:.2} Gbps)", vectored_duration, vectored_throughput);
