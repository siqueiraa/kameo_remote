use kameo_remote::{KeyPair, AuthChallenge, AuthResponse};
...
fn main() {
    ...
    println!("Challenge generated with {} random bytes", challenge.challenge.len());
    ...
    println!("Client signed challenge, claiming PeerId: {}", client_response.peer_id);
    ...
    let mut impostor_response = impostor_keypair.create_auth_response(&challenge.challenge, timestamp);
    impostor_response.peer_id = client_peer_id; // Claim to be client!
    
    println!("Impostor signed with their key but claims to be client: {}", impostor_response.peer_id);
    ...
    let impostor_honest_response = impostor_keypair.create_auth_response(&challenge.challenge, timestamp);
    println!("Impostor with correct PeerId: {}", impostor_honest_response.peer_id);
    ...
    match KeyPair::verify_auth_response(&challenge.challenge, &impostor_honest_response) {
        Ok(()) => println!("✅ Cryptographically valid (but server should reject unauthorized PeerId)"),
