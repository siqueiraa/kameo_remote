...
async fn run_server() -> Result<(), Box<dyn std::error::Error>> {
    ...
    registry.register(
        ...
        "127.0.0.1:28102".parse()?,
    ).await?;
    
    println!("✅ Server listening on {} with PeerId: {}", server_addr, server_peer_id);
    ...
    println!("⚠️  Server expects client with PeerId: {}", expected_client_peer_id);
    ...
    println!("   Only clients with PeerId {} will be accepted", expected_client_peer_id);
...
async fn run_client_with_wrong_key() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = GossipConfig {
        key_pair: Some(wrong_client_keypair),
        ..Default::default()
    };
    ...
    println!("\n✅ Client started on {} with WRONG PeerId: {}", client_addr, wrong_client_peer_id);
    ...
    println!("\n🔗 Attempting to connect to server at {} with WRONG credentials...", server_addr);
    match peer.connect(&server_addr).await {
        Ok(()) => {
            ...
            match registry.lookup("test_actor").await {
                Some(location) => {
                    println!("❌ CRITICAL: Found actor 'test_actor' at {}", location.address);
