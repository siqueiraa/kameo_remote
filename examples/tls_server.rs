...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    registry.register("server.health".to_string(), "127.0.0.1:10001".parse()?).await?;
    println!("   - server.health -> 127.0.0.1:10001");
    
    registry.register("server.metrics".to_string(), "127.0.0.1:10002".parse()?).await?;
    println!("   - server.metrics -> 127.0.0.1:10002");
    
    registry.register("server.config".to_string(), "127.0.0.1:10003".parse()?).await?;
    println!("   - server.config -> 127.0.0.1:10003");
    println!();
    
    println!("💡 Clients can connect using:");
    println!("   cargo run --example tls_client {} {}", port, pub_key_path);
    println!();
