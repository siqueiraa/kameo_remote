...
async fn run_server() -> Result<(), Box<dyn std::error::Error>> {
    ...
    registry.register("auth_test_service".to_string(), "127.0.0.1:29502".parse()?).await?;
    registry.register("secure_data".to_string(), "127.0.0.1:29503".parse()?).await?;
    ...
    loop {
        ...
        println!("📊 Server Status - Active peers: {}, Failed: {}", 
...
async fn run_client_with_valid_keypair() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🔗 Connecting to server at {} (PeerId: {})", server_addr, server_peer_id);
    ...
    match peer.connect(&server_addr).await {
        Ok(()) => {
            ...
            println!("   - Client proved ownership of public key: {}", client_peer_id);
    ...
    println!("   Active peers: {}, Known actors: {}", stats.active_peers, stats.known_actors);
