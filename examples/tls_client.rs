...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        eprintln!("Usage: {} <server_port> [server_pub_key] [client_key] [--wrong-key]", args[0]);
        eprintln!();
        eprintln!("Examples:");
        eprintln!("  {} 9001 /tmp/kameo_tls_server.pub", args[0]);
        eprintln!("  {} 9001 /tmp/kameo_tls_server.pub /tmp/client1.key", args[0]);
        eprintln!("  {} 9001 /tmp/kameo_tls_server.pub --wrong-key", args[0]);
        return Ok(());
    }
    
    let server_port: u16 = args[1].parse()?;
    let server_pub_path = args.get(2)
        .map(|s| s.as_str())
    ...
    let server_node_id = match load_server_node_id(server_pub_path) {
        ...
        Err(e) => {
            eprintln!("❌ Error: Could not load server public key from {}", server_pub_path);
            eprintln!("   {}", e);
            eprintln!();
    ...
    registry.register("client.status".to_string(), "127.0.0.1:11001".parse()?).await?;
    println!("   - client.status -> 127.0.0.1:11001");
    
    registry.register("client.data".to_string(), "127.0.0.1:11002".parse()?).await?;
    println!("   - client.data -> 127.0.0.1:11002");
    println!();
    
    // Add server as peer with its NodeId for TLS verification
    println!("🔗 Adding server as peer at {} with NodeId {}", 
             server_addr, claimed_server_node_id.fmt_short());
    registry.registry.add_peer_with_node_id(server_addr, Some(claimed_server_node_id)).await;
    
    if use_wrong_key {
        println!();
        println!("⏳ Attempting to connect to server (should fail due to public key mismatch)...");
        println!("   We claim the server has NodeId: {}", claimed_server_node_id.fmt_short());
        println!("   But server actually has NodeId: {}", server_node_id.fmt_short());
        ...
        match connection_result {
            Ok(_) => {
                ...
                return Err("Security test failed: connection succeeded with wrong public key".into());
            }
            Err(e) => {
                ...
                if e.to_string().contains("NodeId mismatch") {
                    ...
                    println!("- Server's certificate contains its public key: {}", server_node_id.fmt_short());
                    println!("- We expected a different public key: {}", claimed_server_node_id.fmt_short());
    ...
    if found_count == server_actors.len() {
        ...
        println!();
        println!("⚠️  Only found {}/{} server actors", found_count, server_actors.len());
    ...
    loop {
        ...
        if stats.known_actors > stats.local_actors {
            debug!("Currently know {} actors ({} local, {} remote)", 
...
fn load_server_node_id(path: &str) -> Result<NodeId, Box<dyn std::error::Error>> {
    ...
    if pub_key_bytes.len() != 32 {
        return Err(format!("Invalid public key length: expected 32, got {}", pub_key_bytes.len()).into());
    }
    
    NodeId::from_bytes(&pub_key_bytes)
        .map_err(|e| format!("Invalid NodeId: {}", e).into())
}
